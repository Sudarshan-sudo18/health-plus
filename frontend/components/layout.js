    ...
    <div class="app-frame">
      <header class="topbar">
        ...
        <div class="account-strip">
          ...
          ${user ? `<button class="small-button" type="button" data-action="logout">Logout</button>` : ""}
        </div>
      </header>
      ...
      <footer class="footer">
        <div><strong>Health Plus</strong><span> Full-stack tele-medical platform</span></div>
...
function roleNav(role, activePath) {
  ...
  return items
    .map(([href, label]) => `<a class="nav-link${activePath === href ? " active" : ""}" href="${href}" data-link>${label}</a>`)
    .join("");
}
